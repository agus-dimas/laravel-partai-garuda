        ...
        <main class="flex-1 px-6 py-10 pt-24">
            <div class="mx-auto max-w-3xl">
                <div class="flex items-center justify-between">
                    <div>
                        <h1 class="text-2xl font-semibold text-zinc-900">Customize Struktur</h1>
                        <p class="text-sm text-zinc-500 mt-1">Kelola dan atur konten tampilan halaman Struktur Organisasi.</p>
                    </div>
                </div>
                ...
                <div class="mt-6 rounded-2xl bg-white shadow p-6">
                    <h2 class="text-lg font-semibold text-zinc-900 mb-4 pb-2 border-b border-zinc-100">Section Struktur Organisasi</h2>
                    
                    <form action="{{ route('dashboard.customize.updateStruktur') }}" method="POST" class="space-y-5">
                        @csrf
                        <div>
                            ...
                            <input type="text" name="struktur_section_tagline"
                                value="{{ old('struktur_section_tagline', $settings['struktur_section_tagline']) }}" required
                                class="w-full rounded-lg border border-zinc-200 px-3 py-2 text-sm focus:border-red-500 focus:ring-red-200" />
                            <p class="mt-1 text-xs text-zinc-400">Teks kecil di bagian atas section (Default: Struktur Organisasi)</p>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-zinc-700 mb-1">Judul Section</label>
                            <input type="text" name="struktur_section_title"
                                value="{{ old('struktur_section_title', $settings['struktur_section_title']) }}" required
                                class="w-full rounded-lg border border-zinc-200 px-3 py-2 text-sm focus:border-red-500 focus:ring-red-200" />
                            <p class="mt-1 text-xs text-zinc-400">Judul utama section Struktur Organisasi</p>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-zinc-700 mb-1">Isi Deskripsi Konten</label>
                            <textarea name="struktur_section_description" rows="5" required
                            ...
                            <p class="mt-1 text-xs text-zinc-400">Paragraf penjelasan yang akan tampil di halaman Struktur.</p>
                        </div>
